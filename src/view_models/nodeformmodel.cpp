...
NodeFormModel::NodeFormModel(QObject *parent) :
    QObject(parent), p_trojan(nullptr), p_shadowsocks(nullptr),
    p_vmess(nullptr), p_list(nullptr)
{

}

const QString &NodeFormModel::name() const
{
    return m_name;
}

void NodeFormModel::setName(const QString &newName)
{
    if (m_name == newName)
        return;
...
const QString &NodeFormModel::address() const
{
    return m_address;
}

void NodeFormModel::setAddress(const QString &newAddress)
{
...
int NodeFormModel::port() const
{
    return m_port;
}

void NodeFormModel::setPort(int newPort)
{
    if (m_port == newPort)
        return;
...
const QString &NodeFormModel::password() const
{
    return m_password;
}

void NodeFormModel::setPassword(const QString &newPassword)
{
...
const QString &NodeFormModel::protocol() const
{
    return m_protocol;
}

void NodeFormModel::accept()
{
    NodeInfo node;

    node.name = m_name;
    node.address = m_address;
    node.protocol = magic_enum::enum_cast<EntryType>(m_protocol.toStdString()).value();
    node.port = m_port;
...
void NodeFormModel::setProtocol(const QString &newProtocol)
{
...
TrojanFormModel *NodeFormModel::trojan() const
{
    return p_trojan;
}

void NodeFormModel::setTrojan(TrojanFormModel *newTrojan)
{
...
bool NodeFormModel::setTrojanOutbound(NodeInfo &node)
{
    ...
    outbound_stream.network = magic_enum::enum_cast<TransportObject::Network>(
                p_trojan->network().toStdString()).value();
    ...
    node.raw = QString().fromStdString(outbound_object.toObject().toStyledString());
...
bool NodeFormModel::setVMessOutboud(NodeInfo &node)
{
    if(p_vmess == nullptr) {
        return false;
    }
    ...
    TransportObject::OutboundStreamObject outbound_stream;
    outbound_stream.network = magic_enum::enum_cast<TransportObject::Network>(
                p_vmess->network().toStdString()).value();
...
bool NodeFormModel::setRawOutbound(NodeInfo &node)
{
...
bool NodeFormModel::decodeOutboundFromURL(NodeInfo &node)
{
    bool result = false;
    ...
    do {
        ...
        switch(scheme.value()) {
        case SchemeType::ss:
            setShadowsocksOutboundFromURL(node, url);
            break;
        case SchemeType::vmess:
            setVMessOutboundFromBase64(node, data.toStdString());
            break;
        case SchemeType::trojan:
            setTrojanOutboundFromURL(node, url);
            break;
        default:
            break;
        }
...
bool NodeFormModel::setShadowsocksOutboundFromURL(NodeInfo &node, const QUrl& url)
{
    bool result = false;

    do {
        if (CryptoTools::initAndCheck() < 0) {
            break;
        }
...
bool NodeFormModel::setVMessOutboundFromBase64(NodeInfo &node, const std::string &data)
{
    bool result = false;
...
bool NodeFormModel::setTrojanOutboundFromURL(NodeInfo &node, const QUrl &url)
{
    bool result = false;
...
NodeList *NodeFormModel::list() const
{
    return p_list;
}
...
ShadowsocksFormModel *NodeFormModel::shadowsocks() const
{
    return p_shadowsocks;
}

void NodeFormModel::setShadowsocks(ShadowsocksFormModel *newShadowsocks)
{
...
VMessFormModel *NodeFormModel::vmess() const
{
    return p_vmess;
}

void NodeFormModel::setVmess(VMessFormModel *newVmess)
{
    if (p_vmess == newVmess)
        return;
    p_vmess = newVmess;
...
RawOutboundFormModel *NodeFormModel::raw() const
{
    return p_raw;
}

void NodeFormModel::setRaw(RawOutboundFormModel *newRaw)
{
    if (p_raw == newRaw)
        return;
    p_raw = newRaw;
...
URLSchemeFormModel *NodeFormModel::urlScheme() const
{
    return p_urlScheme;
}

void NodeFormModel::setURLScheme(URLSchemeFormModel *newUrlScheme)
{
