...
void GroupList::init(std::shared_ptr<spdlog::details::thread_pool> thread_pool,
                     DBTools& db,
...
QVector<GroupInfo> GroupList::items() const
{
    return m_items;
}
...
void GroupList::insertSIP008(const std::stringstream& data_stream, const network::CURLTools::DownloadTask& task)
{
...
void GroupList::appendItem(const QString& group_name, const QString& url, int type, int cycle_time)
{
    ...
    if (result == SQLITE_OK) {
        ...
        connect(p_curl, &across::network::CURLTools::downloadFinished, [&]() {
            ...
            if (result == SQLITE_OK) {
                this->insertSIP008(data_stream, task);
            }
        });
...
void GroupList::upgradeItem(int index)
{
...
void GroupList::setDisplayGroupID(int id)
{
...
void GroupList::copyUrlToClipboard(int index)
{
    ...
    QString summary = QString().fromStdString(fmt::format(
        "Copy Group {} url to clipboard", item.name.toStdString()));
