...
ConfigTools::loadDBConfig()
{
    ...
    do {
        ...
        if (temp != m_db.path.toStdString()) {
            setDBPath(QString().fromStdString(temp.value()));
        }
...
void
ConfigTools::setDBPath(const QString& db_path)
{
...
void
ConfigTools::setThemeName(const QString& val)
{
    if (val == m_theme.name) {
        return;
    }
...
QString ConfigTools::extraInfo()
{
    return getExtraInfo();
}

QString ConfigTools::buildTime()
{
    return getBuildTime();
}

QString ConfigTools::sourceCodeURL()
{
