...
namespace network {
    ...
    public:
        ...
        DNSTools(const QString& domain, QDnsLookup::Type type, const QHostAddress& dns_server = QHostAddress::LocalHost);

        void query(const QString& domain, QDnsLookup::Type type, const QHostAddress& dns_server = QHostAddress::LocalHost);
    ...
    public:
        ...
        struct DownloadTask {
            ...
            int64_t group_id;
            std::string url;
            std::string user_agent = "Mozilla/5.0 (Windows NT 10.0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/99.0.7113.93 Safari/537.36";

            DownloadTask(const std::string& name, const std::string& url, int64_t group_id);
        };
    ...
    private:
        ...
        static size_t saveToFileCallback(void* contents, size_t size, size_t nmemb, void* p_ofstream);

        static size_t writeToDataCallback(void* contents, size_t size, size_t nmemb, void* p_data);
    };
}
}
