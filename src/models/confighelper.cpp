...
config::Config ConfigHelper::defaultConfig() {
    auto data_path =
    ...
    config.set_config_version("0.1.1");
    ...
    if (auto interface = config.mutable_interface()) {
        ...
        if (auto theme = interface->mutable_theme()) {
            ...
            theme->set_include_dir(
                (data_dir.filePath("themes") + QDir::separator())
                    .toStdString());
        }
    ...
    if (auto network = config.mutable_network()) {
        ...
        network->set_user_agent("Mozilla/5.0 (Windows NT 10.0) "
...
void ConfigHelper::saveToFile(const std::string &content,
...
std::string ConfigHelper::readFromFile(const QString &file_path) {
