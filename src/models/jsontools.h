...
struct InboundObject {
    ...
    AllocateObject createAllocateObject(const std::string& strategy, int refresh, uint concurrency);
...
struct OutboundObject {
    ...
    void appendShadowsocksObject(ShadowsocksObject::OutboundSettingObject& shadowsocks_object);
    ...
    void setTransportStreamObject(TransportObject::OutboundStreamObject& trojan_stream_object);
    Json::Value toObject();
...
public:
    ...
    static bool jsonParse(const std::string& raw_json, Json::Value& result, Json::String& err_msg);
private:
    std::string m_data = "";
    Json::Value m_root;
};
