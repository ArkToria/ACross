...
void TCPPing::setAddr(const QString& addr)
{
    m_addr = addr;
}

void TCPPing::setAddr(const QHostAddress& addr)
{
    m_addr = addr.toString();
}

void TCPPing::setPort(unsigned int port)
{
    m_port = port;
}

void TCPPing::setTimes(int times)
{
    m_times = times;
}
...
CURLcode CURLTools::download(DownloadTask& task, std::stringstream* p_buf)
{
    auto err = CURLE_OK;

    curl_easy_setopt(task.handle.get(), CURLOPT_URL, task.url.c_str());
    curl_easy_setopt(task.handle.get(), CURLOPT_USERAGENT, task.user_agent.c_str());

    if (p_buf == nullptr) {
        static std::ofstream file(task.filename, std::ios::binary);
        curl_easy_setopt(task.handle.get(), CURLOPT_WRITEFUNCTION, saveToFileCallback);
        curl_easy_setopt(task.handle.get(), CURLOPT_WRITEDATA, reinterpret_cast<void*>(&file));
    } else {
        curl_easy_setopt(task.handle.get(), CURLOPT_WRITEFUNCTION, writeToDataCallback);
...
size_t CURLTools::saveToFileCallback(void* contents, size_t size, size_t nmemb, void* p_ofstream)
{
...
size_t CURLTools::writeToDataCallback(void* contents, size_t size, size_t nmemb, void* p_data)
{
    size_t real_size = size * nmemb;
...
CURLTools::DownloadTask::DownloadTask(const std::string& name, const std::string& url, int64_t group_id)
{
    if (url.empty()) {
        return;
    } else {
    ...
    auto split_lambda = [](std::string_view strv, std::string delims) -> std::vector<std::string_view> {
...
CURLTools::EasyHandle CURLTools::createEasyHandle()
{
